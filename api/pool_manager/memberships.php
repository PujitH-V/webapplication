    ...
    case 'GET':
      ...
      if (isset($_GET['role']) && !empty($_GET['role'])) {
        if ($_GET['role'] == 'visitor') {
          $query = "SELECT pm.id, u.name, u.email, u.phone FROM `pool_members` AS pm
                    JOIN `users` AS u WHERE u.id = pm.member_id 
                    AND u.role = 'visitor' AND pm.adder_id = $user_id";
        }
      } else {
        $query = "SELECT pm.id, u.name, u.email, u.phone FROM `pool_members` AS pm
                  JOIN `users` AS u WHERE u.id = pm.member_id 
                  AND u.role = 'resident' AND pm.adder_id = $user_id";
      }

      $query_result = mysqli_query($conn, $query);
      $pool_members = $query_result->fetch_all();

      $payload = array();
      foreach ($pool_members as $key => $pool_member) {
        array_push($payload, [
          'id'  => intval($pool_member[0]),
          'name'  => $pool_member[1],
          'email'  => $pool_member[2],
          'phone'  => $pool_member[3],
        ]);
    ...
    case 'POST':
      ...
      if (isset($post_body) && !empty($post_body)) {
        ...
        if (mysqli_num_rows($get_user_result) > 0) {
          ...
          $alt_query = "SELECT * FROM `pool_members` WHERE member_id=$member_id
          ...
          $query = "INSERT INTO `pool_members` (
            member_id, 
            adder_id,
            updated_at
          ) VALUES (
          ...
          if (mysqli_query($conn, $query)) {
            ...
            $entry_query = "SELECT pm.id, u.name, u.email, u.phone FROM `pool_members` AS pm
                            JOIN `users` AS u WHERE u.id = pm.member_id 
                            AND pm.id=$entry_id";
            $entry = mysqli_query(
              $conn,
